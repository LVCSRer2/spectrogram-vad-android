...
class VadProbView @JvmOverloads constructor(
    ...
    private val probBuffer = FloatArray(2048) // Safe large buffer for live mode
    ...
    private fun getColsInWindow(windowMs: Int): Int {
        return (windowMs * sampleRate / (1000 * SpectrogramView.SAMPLES_PER_COL))
    }
    ...
    fun clearPlaybackMode() {
        playbackMode = false; playbackProbs = null; totalDurationMs = 0; viewOffsetMs = 0f; currentTimeMs = 0; windowSizeMs = 20000; scroller.forceFinished(true); clear()
    }

    fun setPlaybackMode(enabled: Boolean) { playbackMode = enabled; postInvalidate() }
    ...
    fun addProb(prob: Float) {
        ...
        currentColumn++
        if (currentColumn >= colsIn20s) { currentColumn = 0; wrapped = true }
        postInvalidate()
    }

    fun clear() { currentColumn = 0; wrapped = false; postInvalidate() }

    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        val w = width; val h = height; if (w == 0 || h == 0) return
        canvas.drawRect(0f, 0f, w.toFloat(), h.toFloat(), bgPaint)
        val labelMarginLeft = 100f; val plotW = w - labelMarginLeft
        
        val colsInView = getColsInWindow(if (playbackMode) windowSizeMs else 20000)
        if (colsInView <= 0) return
        val barWidth = plotW / colsInView

        val threshY = h * (1f - speechThreshold); linePaint.color = 0x60FF6B6B.toInt(); canvas.drawLine(labelMarginLeft, threshY, w.toFloat(), threshY, linePaint)

        if (playbackMode && playbackProbs != null) {
            val probs = playbackProbs!!
            val msPerBar = (SpectrogramView.SAMPLES_PER_COL * 1000f / sampleRate)
            val startIdx = (viewOffsetMs / msPerBar).toInt()
            
            for (i in 0 until colsInView) {
                val dataIdx = startIdx + i
                if (dataIdx < 0 || dataIdx >= probs.size) continue
                val prob = probs[dataIdx]
                if (prob <= 0.001f) continue
                val barH = prob * h; val left = labelMarginLeft + (i * barWidth); val right = left + barWidth
            ...
            for (i in 0 until colsInView) {
                val bufferIdx = if (!wrapped) { if (i >= currentColumn) continue; i } else { (currentColumn + i) % colsInView }
                val prob = probBuffer[bufferIdx]; if (prob <= 0.001f) continue
                val barH = prob * h; val left = labelMarginLeft + (i * barWidth); val right = left + barWidth
                barPaint.color = if (prob >= speechThreshold) 0xCCFF6B6B.toInt() else 0x664CAF50.toInt()
                canvas.drawRect(left, h - barH, right, h.toFloat(), barPaint)
            }
